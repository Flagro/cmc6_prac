...
void run_tests() {
    ...
    double result_parallel = dot_product<double>(X, Y, &elapsed_time, false, &bandwidth);
    double result_sequential = dot_product_sequential<double>(X, Y);
    ...
    DenseVector<double> vector_result_parallel = SpMV<double>(spmv_a, spmv_b, &elapsed_time, false, &bandwidth);
    DenseVector<double> vector_result_sequential = SpMV_sequential<double>(spmv_a, spmv_b);
    ...
    DenseVector<double> axpby_result_parallel = axpby<double>(axbpy_a, X, axbpy_b, Y, &elapsed_time, false, &bandwidth);
    DenseVector<double> axpby_result_sequential = axpby_sequential<double>(axbpy_a, X, axbpy_b, Y);
