...
std::vector<MatrixGenerator<double> > matrix_function_generators = {
    {[](size_t i, size_t j, [[maybe_unused]] size_t n) { return rand() % 12381245823; }, "A[i, j] = rand() % 12381245823"},
    {[](size_t i, size_t j, [[maybe_unused]] size_t n) { return (i * 37) % 5 + j * 13; }, "A[i, j] = (i * 37 + j * 13)"},
    {[](size_t i, size_t j, [[maybe_unused]] size_t n) { return ((i * 137) % 47 + j * 13 + 176584) % 97; }, "A[i, j] = ((i * 137) % 47 + j * 13 + 176584) % 97"}
};

std::vector<VectorGenerator<double> > vector_function_generators = {
    {[](size_t i, [[maybe_unused]] size_t n) { return rand() % 12381245823; }, "b[i] = rand() % 12381245823"},
    {[](size_t i, [[maybe_unused]] size_t n) { return 11 * i + 17; }, "b[i] = (11 * i) + 17"},
    {[](size_t i, [[maybe_unused]] size_t n) { return ((159 * i) % 13 + 17) % 57; }, "b[i] = ((159 * i) % 13 + 17) % 57"}
};
